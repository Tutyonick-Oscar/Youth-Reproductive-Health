...
@section('title','Youth Reproductive Health | Causes')
@section('content')
  ...
  <div class="flex-1 main">
    ...
    <section class="flex flex-col gap-4 px-8 py-5 ">
        ...
        <div class="flex flex-col w-full gap-6 ">
            <h1 class="text-2xl ">
                {{isset($cause) ? 'METTRE A JOUR LA CAUSE' : 'AJOUTER UNE CAUSE'}}
            </h1>
        </div>
       </section>
       <section class="flex flex-col w-full gap-4 px-8 pb-5">
        <form id="form" method="POST" action="{{isset($cause) ? route('admin.updateCauses',['id' => $cause->id]) : route('admin.storeCauses')}}" class="w-full flex gap-10 items-center h-[25.75rem] justify-center" enctype="multipart/form-data">
            @csrf
            @if (isset($cause))
                @method('PUT')
            @endif
            ...
            <div class="w-full flex flex-col gap-3 h-[25.75rem]">
                <div class=" w-full flex gap-4 px-4 py-4 rounded-[4px] bg-[#fff] items-center">
                    ...
                    <input type="text" name="title" value="{{isset($cause) ? $cause->title : old('title')}}"
