...
@section('title','Youth Reproductive Health | Eevents management')
@section('content')
<div class="w-full h-auto md:flex">
  ...
  <div class="flex-1 main">
   ...
   <section class="flex flex-col gap-4 px-8 py-5 ">
    <div class="w-full ">
        <p> Accueil / Gestionnaire des evenements</p>
    </div>
    <div class="flex flex-col w-full gap-6 ">
        <h1 class="text-2xl ">
            GESTIONNAIRE DES EVENEMENTS
        </h1>
        <p class=" rounded-md px-4 py-1 cursor-pointer bg-secodary text-white text-lg w-[10rem]">
            <a href="{{route('admin.addEvent')}}" class="w-full text-base ">
                Ajouter evenement
            </a>
        </p>
    </div>
        ...
        <table class=" dash-border rounded-[4px]">
            <thead class="border-b dash-border">
                <tr class="">
                    ...
                    <th class="w-[60%] border-r dash-border">Titre de l'evenement</th>
            ...
            <tbody>
                ...
                <tr class="h-[2rem]">
                    <td class="w-[8%]  border-r dash-border h-[4rem] text-center"> {{$event->id}} </td>
                    <td class="w-[12%] border-r dash-border  py-2 h-[50px] px-4"> 
                        <img loading="lazy" decoding="async" src="/storage/{{$event->image}}" alt="event img" width="80" height="50" class="
                        w-full h-[50px] object-cover img rounded-md">
                    </td>
                    <td class="w-[60%] border-r dash-border text-center "> {{$event->title}} </td>
                    <td class="w-[10%] border-r dash-border text-center ">
                         {{isset($event->date) ? $carbon::parse($event->date)->format('d-m-Y') : $event->created_at->format('d-m-Y')}} 
                        </td>
                    <td class="px-2 dash-border">
                        <div class="flex items-center justify-center w-full gap-3 ">
                            <a href="{{route('admin.getEvent',['id'=>$event->id])}}" class="flex items-center justify-center gap-1 px-2 py-1 text-base border rounded-md border-bg text-bg">
                                <i class="fa-solid fa-pen"></i>
                                Editer
                             </a>
                             <form method="POST" action="{{route('admin.deleteEvent',['id'=>$event->id])}}" class="flex items-center ">
                                @csrf
                                @method('DELETE')
    ...
    <div class=" w-full flex justify-between items-center bg-[#fff] py-4 px-4 rounded-sm">
        <div>
            <p class="text-base text-bg">
                {{$events->firstItem()}} à {{$events->lastItem()}} résultats sur {{count($events)}}
            </p>
        </div>
        <div class="flex ">
