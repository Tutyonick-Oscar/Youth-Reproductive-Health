...
<div class="w-full md:flex h-auto">
  ...
  <div class="flex-1 main">
   ...
   <section class=" flex flex-col gap-4 py-5 px-8">
    <div class=" w-full">
        <p> Accueil / Gestionnaire des causes</p>
    </div>
    <div class=" flex flex-col gap-6 w-full">
        <h1 class=" text-2xl">
            GESTIONNAIRE DES CAUSES
        </h1>
        <p class=" rounded-md px-4 py-1 cursor-pointer bg-secodary text-white text-lg w-[9rem]">
            <a href="{{route('admin.addCauses')}}" class=" w-full text-base">
                Ajouter une cause
            </a>
        </p>
    </div>
   ...
   <section class=" w-full px-8 flex flex-col">
        <table class=" dash-border rounded-[4px]">
            ...
            <tbody>
                ...
                <tr class="h-[2rem]">
                    <td class="w-10  border-r dash-border h-[4rem] text-center"> {{$cause->id}} </td>
                    <td class="w-[10rem] border-r dash-border  py-2 h-[50px] px-2"> 
                        <img loading="lazy" decoding="async" src="/storage/{{$cause->image}}" alt="" width="80" height="50" class="
                        w-[90%] h-[50px] object-cover img rounded-md">
                    </td>
                    <td class="w-[35rem] border-r dash-border text-center "> {{$cause->title}} </td>
                    <td class="dash-border px-2">
                        <div class=" flex gap-3 w-full justify-center items-center">
                            <a href="{{route('admin.getCauses',['id'=>$cause->id])}}" class=" border border-bg rounded-md flex justify-center items-center px-2 py-1 gap-1 text-base text-bg">
                                <i class="fa-solid fa-pen"></i>
                                Editer
                             </a>
                             <form action="{{route('admin.deleteCauses',['id'=>$cause->id])}}" class=" flex items-center" method="POST">
                                @csrf
                                @method('delete')
