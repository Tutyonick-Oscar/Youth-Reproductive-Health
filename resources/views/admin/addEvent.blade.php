...
@section('title','Rédiger un Evenement')
@section('content')
<div class="w-full h-auto md:flex">
  ...
  <div class="flex-1 main">
    ...
    <section class="flex flex-col gap-4 px-8 py-5 ">
        <div class="w-full ">
            <p> Accueil / Event management </p>
        </div>
        <div class="flex flex-col w-full gap-6 ">
            <h1 class="text-2xl ">
                {{isset($event) ? 'METTRE A JOUR L\'EVENEMRNT' : 'AJOUTER UN EVENEMENT'}}
            </h1>
        </div>
       </section>
       <section class="flex flex-col w-full gap-4 px-8 pb-5">
        <form id="form" method="POST" action="{{isset($event) ? route('admin.updateEvent',['id'=>$event->id]) : route('admin.createEvent')}}" method="POST" class="w-full flex gap-10 items-center h-[25.75rem] justify-center" enctype="multipart/form-data">
            @csrf
            @if (isset($event))
                @method('PUT')
            @endif
            <div id="klk" class="flex flex-col h-full gap-4 ">
                ...
                <input id="image" type="file" name="image" class="hidden" onchange="previewPhoto()" value="old('image')">
                ...
                <div class="relative w-full flex gap-2 px-4 py-4 rounded-[4px] bg-[#fff] items-center">
                    ...
                    <input id="datepicker" value="{{isset($event) ? $event->date : old('date')}}"
                ...
                <div class=" w-full flex gap-2 px-4 py-4 rounded-[4px] bg-[#fff] items-center">
                    <label for="type" class="text-base font-bold text-bg">Statut : </label>
                    <select name="status" class="px-4 py-1 rounded-[4px]">
                        <option class="px-4 py-1" value="upcoming">A venir</option>
                        <option value="past">Passe</option>
                    </select>
                    {{-- <input  type="text" name="status" value="{{isset($event) ? $event->status : old('status')}}" class=" outline-none duration-300 ease-in-out dash-border rounded-[4px] px-4 py-1 focus:border-accent1"> --}}
                    @error('status')
                        <small class="text-red">
                            {{$message}}
                        </small> 
            ...
            <div class="w-[70%] flex flex-col gap-3 h-[25.75rem]">
                <div class=" w-full flex gap-4 px-4 py-4 rounded-[4px] bg-[#fff] items-center">
                    ...
                    <input  type="text" name="title" value="{{isset($event) ? $event->title : old('title')}}" class="w-[90%] outline-none duration-300 ease-in-out dash-border rounded-[4px] px-4 py-1 focus:border-accent1">
                    @error('title')
                        <small class="text-red">
                            {{$message}}
                        </small> 
                ...
                <div class=" w-full flex gap-2 px-4 py-4 rounded-[4px] bg-[#fff] items-center">
                    ...
                    <div id="tagsDiv" class="flex gap-2 ">
                        @if (isset($event))
                            @foreach (explode(',',$event->tags) as $item)
                ...
                <div class="flex gap-3 ">
                    <button type="submit" id="submit" class=" cursor-pointer px-4 py-1 rounded-[4px] bg-accent1 text-white text-base font-semibold">
                        {{isset($event) ? 'Mettre à jour' : 'Envoyer'}}
                    </button>
                    <button type="reset" class="px-4 py-1 rounded-[4px] dash-border cursor-pointer">
                        Reset
                    </button>
                </div>
            </div>
